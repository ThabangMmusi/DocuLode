...
class TextBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isCompact) {
      ...
      final TextStyle? currentDefaultTextStyle =
          effectiveStyle.textStyle?.resolve({});
      effectiveStyle = effectiveStyle.copyWith(
        padding: WidgetStateProperty.all(
          EdgeInsets.symmetric(
            horizontal: (currentDefaultPadding.horizontal * 0.75)
                .clamp(Insets.xs, Insets.sm),
            vertical: (currentDefaultPadding.vertical * 0.75)
                .clamp(Insets.xs / 2, Insets.xs),
          ),
        ),
        textStyle: WidgetStateProperty.all(
            (currentDefaultTextStyle ?? theme.textTheme.labelLarge)
                ?.copyWith(fontSize: theme.textTheme.labelMedium?.fontSize)),
      );
    }
