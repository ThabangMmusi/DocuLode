...
class CloudStorageService {
  ...
  Future<List<CloudinaryResponse>> multiUpload(
    ...
    if (images.first.path != null) {
      futures = images.map((img) async {
        return CloudinaryFile.fromFile(img.path!);
      }).toList();
    }
    // TODO: Check if we still need assets/bytedata
    // Handle images as "asset" files ios/android
    // else if (images.first.asset != null) {
    //   print("Upload from future bytes... ");
    //   futures = images.map((image) {
    //     return CloudinaryFile.fromFutureByteData(image.asset!.readAsBytes(), identifier: image.asset!.identifier!);
    //   }).toList();
    // }
  ...
  static void addMaxSizeToUrlList<T>(
      List<T> items, String Function(T) getUrl, T Function(T, String) setUrl) {
